...
export async function withEvalRetries(name, attemptFn) {
  const maxRetries = 3;
  let attempt = 0;
...
export async function internalEvalTest(evalCase) {
 ...
 */
export async function prepareWorkspace(testDir, homeDir, files) {
  const acknowledgedAgents = {};
  ...
  const execOptions = { cwd: testDir, stdio: 'ignore' };
 ...
 */
export function runEval(policy, evalCase, fn, timeoutOverride) {
...
export async function prepareLogDir(name) {
 ...
 */
export function symlinkNodeModules(testDir) {

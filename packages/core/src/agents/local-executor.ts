...
import { constructOrchestrationPrompt } from './orchestration.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private async buildSystemPrompt(inputs: AgentInputs): Promise<string> {
    ...
    // Append Team Orchestration Context if this agent is a lead
    const teamRegistry = this.context.config.getTeamRegistry();
    const currentTeamName = this.context.config.getCurrentTeam();
    const team = currentTeamName
      ? teamRegistry.getDefinition(currentTeamName)
      : teamRegistry
          .getAllDefinitions()
          .find((t) => t.leadAgent === this.definition.name);

    if (team && team.leadAgent === this.definition.name) {
      const agentRegistry = this.context.config.getAgentRegistry();
      const members = team.members
        .map((m) => agentRegistry.getDefinition(m))
        .filter((m): m is AgentDefinition => m !== undefined);
      finalPrompt += `\n\n${constructOrchestrationPrompt(team, members)}`;
    }

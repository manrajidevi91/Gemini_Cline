...
describe('Plan Mode', () => {
  ...
  it('should allow write_file to the plans directory in plan mode', async () => {
    ...
    expect(
      ...
      `Expected write_file to succeed, but it failed with error: ${'error' in (planWrite?.toolRequest || {}) ? (planWrite?.toolRequest)['error'] : 'unknown'}`,
    ).toBe(true);
  });
  ...
  it('should switch from a pro model to a flash model after exiting plan mode', async () => {
    ...
    const modelNames = apiRequests.map(
      (r) =>
        ('model' in (r.attributes || {}) ? r.attributes['model'] : 'unknown') ||
        'unknown',
    );
